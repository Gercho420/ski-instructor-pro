...
export default function Services() {
  const { t } = useI18n();
  // Blindaje para consultas de tRPC o configuraciones de servicios
      ...
      <div className="container mx-auto max-w-6xl">
        ...
        {isLoading ? (
          <div className="grid md:grid-cols-3 gap-6">
            {Array.from({ length: 3 }).useQuery ? null : Array.from({ length: 3 }).map((_, i) => (
              <Skeleton key={i} className="h-64 rounded-lg" />
            ))}
          </div>
        ) : services.length === 0 ? (
          <div className="grid md:grid-cols-3 gap-6">
            {/* Tarjetas por defecto de respaldo para que la sección nunca quede vacía ni falle */}
            ...
            <div className="corner-bracket p-8 bg-[oklch(0.97_0.012_300/0.5)] backdrop-blur-sm rounded-lg border border-[oklch(0.90_0.02_300/0.3)] flex flex-col justify-between">
              <div>
                <h3 className="font-serif text-2xl text-[oklch(0.30_0.05_295)] mb-3">Clinicas en Grupo</h3>
                <p className="text-sm font-sans font-light text-[oklch(0.45_0.04_295)] leading-relaxed mb-6">
